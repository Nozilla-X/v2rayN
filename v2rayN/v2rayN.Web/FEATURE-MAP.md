...
The Backend project is `v2rayN/v2rayN.Web/`; the Vue source is nested at `v2rayN/v2rayN.Web/WebUI/`. ServiceLib remains the shared business layer. Web scheduling and its cancellation lifecycle live in `V2rayRuntime.Scheduling.cs`; ServiceLib's desktop `TaskManager` is not registered by the Web host. Web's `RuntimeMutationGate` serializes Web-side writes to shared config and SQLite state. The Linux TUN launch policy is implemented in `ServiceLib.Manager.CoreManager`: root requires effective `CAP_NET_ADMIN` (`CapEff`), while non-root requires ambient `CAP_NET_ADMIN` (`CapAmb`) to survive child `exec`. WPF/Avalonia views are unchanged. The Web workspace is intentionally a compact, profile-table-first frontend rather than a separate proxy Dashboard.
...
| TUN toggle and TUN settings | `Config.TunModeItem`; `CoreConfigContextBuilder`; Core inbound/config builders; `StatusBarViewModel`; `OptionSettingViewModel` | `GET /api/settings` and `GET/PUT /api/settings/tun`; `GET /api/status` reports `tunEnabled` | Main runtime strip toggle plus TUN settings panel. Enabling checks Linux `/dev/net/tun` access and the launch credential: root requires `CAP_NET_ADMIN` in `CapEff`; non-root requires it in `CapAmb` so it survives child `exec`. Missing/unreadable capability state is unavailable; containers without the required capability receive an explicit response. ServiceLib still owns config generation and Core restart. |
...
| DNS: simple DNS and per-Core DNS profiles | `Config.SimpleDNSItem`; `DNSItem`; `ConfigHandler.SaveDNSItems/GetExternalDNSItem` | `GET/PUT /api/settings/dns/simple`; `GET/PUT /api/settings/dns/profiles` | DNS section edits simple DNS and per-Core DNS including `TunDNS`; TUN DNS remains ServiceLib-owned configuration |
| Full Xray/sing-box config templates | `FullConfigTemplateItem`; `ConfigHandler.SaveFullConfigTemplate` | `GET /api/settings/core-templates`; `PUT /api/settings/core-templates/{coreType}` | Advanced Core template editor exposes both regular `Config` and `TunConfig` fields |
...
The profile list stays above the fold and is the primary surface. Row activation, context actions, group selection, column sort, batch operations, and speed-test flows are retained. Settings follow the original inbound/TUN/Core/General/Speedtest, Routing, DNS, and template groupings. System proxy remains desktop-specific; TUN stays available through Web API/UI and reports missing host capability instead of hiding the feature.
