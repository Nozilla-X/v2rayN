...
<section class="page logs-page">
  <div class="page-toolbar"><div class="page-title"><h1>{{ t('logs.title') }}</h1><span class="count-tag">{{ state.logTotal }}</span></div><div class="toolbar-main"><label class="search-box log-search"><span>⌕</span><input v-model="state.logFilter" :placeholder="t('logs.filter')" @keyup.enter="actions.loadLogs(1)" /></label><button class="button" @click="actions.loadLogs(1)">{{ t('logs.load') }}</button><button class="button danger" @click="actions.clearLogs">{{ t('logs.clear') }}</button></div></div>
  <div ref="logPanel" class="log-table-wrap"><table class="data-table log-table"><thead><tr><th>{{ t('logs.time') }}</th><th>{{ t('logs.source') }}</th><th>{{ t('logs.message') }}</th></tr></thead><tbody><tr v-for="(log, index) in state.logs" :key="`${log.timestamp}-${index}`"><td class="log-time">{{ new Date(log.timestamp).toLocaleTimeString(locale, { hour12: false }) }}</td><td><span class="source-tag">{{ log.source }}</span></td><td class="log-message">{{ log.message }}</td></tr><tr v-if="!state.logs.length"><td colspan="3" class="empty-row">{{ t('logs.noLogs') }}</td></tr></tbody></table></div>
