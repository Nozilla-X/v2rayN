...
<template>
<section class="page">
  <div class="page-toolbar"><div class="page-title"><h1>{{ t('subscriptions.title') }}</h1><span class="count-tag">{{ state.subscriptions.length }}</span></div><div class="toolbar-main"><label class="check-inline"><input v-model="state.subscriptionUseProxy" type="checkbox" />{{ t('subscriptions.useProxy') }}</label><button class="button" @click="actions.updateSubscriptions(null, state.subscriptionUseProxy)">{{ t(actions.subscriptionUpdateMessageKey(null, state.subscriptionUseProxy)) }}</button><button class="button" @click="actions.updateSubscriptions(state.selectedGroup || null, state.subscriptionUseProxy)">{{ t(actions.subscriptionUpdateMessageKey(state.selectedGroup || null, state.subscriptionUseProxy)) }}</button><button class="button primary" @click="actions.openAddSubscription">＋ {{ t('subscriptions.addSubscription') }}</button></div></div>
