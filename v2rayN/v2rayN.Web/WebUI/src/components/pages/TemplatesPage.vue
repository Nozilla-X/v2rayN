...
<template>
  <section class="page templates-page">
    ...
    <form v-if="currentTemplate" class="template-editor" @submit.prevent="actions.saveTemplate(currentTemplate)">
      ...
      <footer class="settings-footer"><span class="muted">{{ t('templates.templateHint') }}</span><button class="button primary" type="submit">{{ t('common.save') }}</button></footer>
    </form>
