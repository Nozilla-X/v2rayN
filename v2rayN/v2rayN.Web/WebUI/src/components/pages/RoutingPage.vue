  ...
  <section class="page routing-page">
    ...
    <div class="split-workspace routing-workspace">
      <section class="subpanel route-list-panel">
        <div class="subpanel-heading"><div class="heading-check"><input type="checkbox" :checked="allRoutesSelected" :aria-label="t('common.selectAll')" @change="actions.toggleAllRoutes" /><h2>{{ t('routing.profiles') }}</h2><span class="count-tag">{{ state.routes.length }}</span></div><div class="row-actions"><button class="tool-button" :title="t('common.selectAll')" @click="actions.toggleAllRoutes">☑</button><button class="tool-button" :title="t('common.refresh')" @click="actions.loadRouting">↻</button></div></div>
        <div v-for="route in state.routes" :key="route.id" :class="['route-row', { selected: route.id === state.activeRoutingId, current: route.isActive }]">
          ...
          <button class="route-select" @click="state.activeRoutingId = route.id; actions.loadRules(route.id)"><span class="route-info"><strong>{{ route.remarks }}</strong><small>{{ route.ruleNum }} · {{ route.enabled ? t('common.enabled') : t('common.disabled') }}</small></span><span v-if="route.isActive" class="current-label">{{ t('routing.default') }}</span></button>
          <div class="row-actions"><button class="tool-button" :title="t('common.edit')" @click="actions.openEditRoute(route)">✎</button><button class="tool-button danger-text" :title="t('common.delete')" @click="actions.deleteRoute(route)">×</button></div>
        </div>
      ...
      <section class="subpanel rules-panel">
        <div class="subpanel-heading rules-heading"><div><h2>{{ t('routing.rules') }}</h2><small>{{ state.selectedRoute?.remarks || t('common.none') }} · {{ state.routingRules.length }}</small></div><button class="button compact primary" :disabled="!state.activeRoutingId" @click="actions.addRoutingRule">＋ {{ t('routing.addRule') }}</button></div>
        ...
        <div class="rules-mini-table">
          ...
          <div v-for="rule in state.routingRules" :key="rule.id" class="rule-row" :class="{ selected: state.selectedRuleIds.includes(rule.id) }" @dblclick="actions.openEditRoutingRule(rule)">
            ...
            <button class="rule-edit-button" :title="t('common.edit')" @click="actions.openEditRoutingRule(rule)"><span :class="['rule-state', { off: !rule.enabled }]">{{ rule.enabled ? '●' : '○' }}</span><strong>{{ rule.remarks || '—' }}</strong></button>
            ...
            <div class="row-actions"><button class="tool-button" :title="t('common.edit')" @click="actions.openEditRoutingRule(rule)">✎</button><button class="tool-button danger-text" :title="t('common.delete')" @click="actions.removeRoutingRule(rule)">×</button></div>
          </div>
