...
<template>
  <div v-if="state.showImportForm" class="modal-shade" @click.self="state.showImportForm = false">
    <form class="modal-panel wide-modal modal-form" @submit.prevent="actions.importProfiles">
      <header class="modal-head"><h2>{{ t('nodes.importNodes') }}</h2><button class="tool-button" type="button" :aria-label="t('common.close')" @click="state.showImportForm = false">×</button></header>
